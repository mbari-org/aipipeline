...
# Filename: aipipeline/metrics/gen_tsne_plot.py
# Description: Plot t-SNE of vectors from Redis
...
# Helper function to convert Redis binary vector to a list of floats
...
# Download all vectors from Redis
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "tator": {"project": "901103-biodiversity"},
    }
    password = os.getenv("REDIS_PASSWD")
    if not password:
        logging.error("REDIS_PASSWD not found. Need to set in .env file")
        exit(-1)

    plot_tsne(config, password)
