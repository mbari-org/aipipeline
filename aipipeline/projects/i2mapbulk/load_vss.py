...
# find /mnt/DeepSea-AI/data/i2map/processed/vss -type f -name "*.json" &> i2mapbulkjson.txt
# find /mnt/DeepSea-AI/data/m3/processed/vss -type f -name "*.json" &>  m3mapbulkjson.txt
#
# And export all the Baseline data in i2mapbulk to a CSV file
# python download_baseline.py
#
# Then run this script, e.g.
# python load_vss.py --input_json_paths i2mapbulkjson.txt --save_path /tmp/i2mapbulkvss_top3 --tator_loaded_csv tator_i2mapbulk_baseline.csv --version mbari-i2map-vits-b8-20251008-vss
# python load_vss.py --input_json_paths m3mapbulkjson.txt --save_path /tmp/m3mapbulkvss_top3 --tator_loaded_csv tator_i2mapbulk_baseline.csv --version mbari-m3-vits-b8-20251011-vss
import argparse
import os
import subprocess
import sys
...
# and log to file
now = datetime.now()
log_filename = f"i2mapbulk_load_vss{now:%Y%m%d}.log"
...
if __name__ == "__main__":
    ...
    config = "https://docs.mbari.org/internal/ai/projects/config/config_i2mapbulk.yml"
