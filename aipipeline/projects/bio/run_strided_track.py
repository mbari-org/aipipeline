...
def run_inference_track_parallel(args):
    video_file, version_id, kwargs = args
    run_inference_track(video_file, version_id, **kwargs)

def run_inference_track(video_file: str, version_id: int, **kwargs):
    """
    ...
    det_batch_size = kwargs.get("det_batch_size", 1)
    ...
    if endpoint_url:
        ...
        yv5 = YV5(det_model, device_num=gpu_id)
    ...
    for frame_num in range(0, total_frames, frame_stride):
                ...
                for track in closed_tracks:
                    ...
                    if not skip_load:
                        ...
                        new_loc = {
                            ...
                            "cluster": "-1",
                            "label": best_labels[0],
                            "label_s": best_labels[1] if best_labels[1] else "marine organism",
                            "dive": md["dive"],
        ...
        for loc in reversed(data):
            ...
            if has_gpu:
                crop_square_image_gpu(loc, 224, gpu_id)
            else:
                crop_square_image(loc, 224)
...
def process_videos(
    ...
    # If we are using a gpu, we need to set the start method to forkserver
    if torch.cuda.is_available():
        multiprocessing.set_start_method("forkserver")
    else:
        multiprocessing.set_start_method("spawn")
    video_files_args = [ (video_path, version_id, kwargs) for video_path in video_files]
    with multiprocessing.Pool(12) as pool:
        pool.map(run_inference_track_parallel, video_files_args)
...
def parse_args():
    ...
    parser.add_argument(help="TSV file with video paths per Haddock output", required=False, type=str)
    ...
    parser.add_argument("--det-model",help="Object detection vmodel path.",required=False,type=str,)
    parser.add_argument("--det_batch_size", help="Batch size for object detection.", default=1, type=int)
...
if __name__ == "__main__":
    ...
    if args.video:
        ...
        if video_path.is_file():
            ...
            video_files = [v.as_posix() for v in video_files]
        ...
        # Make sure the files are unique - there may be duplicates, and we don't want to process them multiple times
        video_files = list(set(video_files))
        video_files = [resolve_video_path(Path(v)) for v in video_files]
        video_files = [v.as_posix() for v in video_files if v is not None]
        process_videos(video_files, version_id, **args_dict)
